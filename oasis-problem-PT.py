...
def problem_parameters(commandline_kwargs, NS_parameters, **NS_namespace):
    ...
    if restart_folder:
        ...
        cycles    = get_cmdarg(commandline_kwargs, 'cycles', 2)
        ...
        case_fullname = (mesh_name + "_ts" + str(timesteps) + "_cy" + str(cycles))
        ...
        NS_parameters.update(
            ...
            cycles              = cycles,                                           # total number of cycles
            period              = period,                                           # time of a single cycle [ms]
            T                   = period * cycles,                                  # total simulation time [ms]
    ...
    g = len(str(int(NS_parameters['time_steps']*NS_parameters['cycles'])))+1
...
def create_bcs(u_, p_, p_1, t, NS_expressions, V, Q, area_ratio, mesh, subdomain_data, 
               ...
               id_in, id_out, velocity_degree, pressure_degree, cycles,
...
def pre_solve_hook(mesh, V, Q, newfolder, results_folder, u_, mesh_path,
    ...
    return dict(hdf5_link=h5stdio,
                ...
                timesteps=NS_namespace['time_steps'], total_cycles=NS_namespace['cycles'],
...
def temporal_hook(u_, p_, p, q_, V, mesh, tstep, compute_flux,
    ...
    if save_first_cycle:
        ...
        if tstep in [8850, 17753, 26656, 35559, 44462, 53365]:
